...
void PolymerObservable::writeMapDotProduct(Polymer& polymer, char* fileName)
{
    int i, j;
    int frameSize, framePosition;
    double answ = 0.0;
    ...
    fp = fopen(fileName, "w");
    if(fp == NULL){
	printf("Error in writeMapDotProduct:\nCannot create the file '%s'\n",fileName);
	exit(1);
    }
    
    if(polymer.getVectorsB() == NULL)
	polymer.setVectorsBfromVectorsT();
	...
	for(framePosition=0;framePosition<=numMonomers-frameSize;framePosition++){
	    tmpVectorsT = new Vector [frameSize];
	    tmpVectorsB = new Vector [frameSize];

	    for(i=0;i<frameSize;i++){
		tmpVectorsT[i] = t[i+framePosition];
		tmpVectorsB[i] = b[i+framePosition];
	    }

	    tmpPolymer = new Polymer(frameSize, NULL, tmpVectorsT, tmpVectorsB);
	    T = new Vector;
	    B = new Vector;
	    

	    Vector::KadanoffTransformation(frameSize, T, t);
	    Vector::KadanoffTransformation(frameSize, B, b);

	    answ = Vector::dotProduct(*T,*B);
	    fprintf(fp,"%i\t%i\t%.15le\n",framePosition,frameSize, answ);
