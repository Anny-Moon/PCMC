...
//    printf("num mon %i\n",numMonomersInFile);
    ...
    sprintf(polymer1FileName,"%s.dat",p[2]);
    FileAngles reader1(polymer1FileName,0);
    Polymer plm1(reader1);

    if(numMonomersInFile != plm1.getNumMonomers()){
	if(myCoreNumber==0){
    	    printf("\tError: number of monomers in .pcmc file '%s' and in .dat file '%s' does not match!\n", paramFileName, polymer1FileName);
    	    printf("\tIt is %i and %i correspondently.",numMonomersInFile,plm1.getNumMonomers());
    	}
    	exit(1);
    }

    plm1.setVectorsTNBfromKappaTau();
    plm1.setRadiusVectorsFromVectorsT();
    
    polymer2FileName = new char[1000];
    sprintf(polymer2FileName,"%s.dat",p[3]);
    FileAngles reader2(polymer2FileName,0);
    ...
    plm2.setVectorsTNBfromKappaTau();
    plm2.setRadiusVectorsFromVectorsT();
